...
namespace BlackCat\Database\Packages\RateLimitCounters\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto;
use BlackCat\Database\Packages\RateLimitCounters\Mapper\RateLimitCounterDtoMapper;
 ...
 */
final class RateLimitCountersAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RateLimitCounterRepository $rateLimitCounterRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rate_limit_counters' => $this->rateLimitCounterRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RateLimitCounters\ModuleException("Record {$idStr} not found for locking.");
            }
