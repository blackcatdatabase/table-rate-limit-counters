...
namespace BlackCat\Database\Packages\RateLimitCounters\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RateLimitCounters\Definitions;
use BlackCat\Database\Packages\RateLimitCounters\Criteria;
use BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto as Dto;
use BlackCat\Database\Packages\RateLimitCounters\Mapper\RateLimitCounterDtoMapper as RowMapper;
...
final class RateLimitCounterRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RateLimitCounters\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'subject_type', 'subject_id', 'name', 'window_start', 'window_size_sec' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RateLimitCounters\Definitions::table());
            return $bulk->upsertMany($rows, [ 'subject_type', 'subject_id', 'name', 'window_start', 'window_size_sec' ], [ 'count', 'updated_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto|null */
    public function getBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec(string $subjectType, string $subjectId, string $name, \DateTimeInterface|string $windowStart, int $windowSizeSec, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto|null {
        $row = $this->getByUnique([ 'subject_type' => $subjectType, 'subject_id' => $subjectId, 'name' => $name, 'window_start' => $windowStart, 'window_size_sec' => $windowSizeSec ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RateLimitCounters\Mapper\RateLimitCounterDtoMapper::fromRow($row);
    }
    public function existsBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec(string $subjectType, string $subjectId, string $name, \DateTimeInterface|string $windowStart, int $windowSizeSec): bool {
        $where = 't.' . Ident::q($this->db, 'subject_type') . ' = :uniq_subject_type' . ' AND ' . 't.' . Ident::q($this->db, 'subject_id') . ' = :uniq_subject_id' . ' AND ' . 't.' . Ident::q($this->db, 'name') . ' = :uniq_name' . ' AND ' . 't.' . Ident::q($this->db, 'window_start') . ' = :uniq_window_start' . ' AND ' . 't.' . Ident::q($this->db, 'window_size_sec') . ' = :uniq_window_size_sec';
        return $this->exists($where, [ 'uniq_subject_type' => $subjectType, 'uniq_subject_id' => $subjectId, 'uniq_name' => $name, 'uniq_window_start' => $windowStart, 'uniq_window_size_sec' => $windowSizeSec ]);
    }
    /** @return int|string|null */
    public function getIdBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec(string $subjectType, string $subjectId, string $name, \DateTimeInterface|string $windowStart, int $windowSizeSec) {
        $row = $this->getBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec($subjectType, $subjectId, $name, $windowStart, $windowSizeSec);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto|null {
