...
use BlackCat\Database\Packages\RateLimitCounters\Repository\RateLimitCounterRepositoryInterface;
...
class RateLimitCounterRepository implements RateLimitCounterRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'subject_type', 'subject_id', 'name', 'window_start', 'window_size_sec' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RateLimitCounters\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'count', 'updated_at' ]);
          return count($rows);
        }
    ...
    public function getBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec(string $subjectType, string $subjectId, string $name, \DateTimeInterface|string $windowStart, int $windowSizeSec, bool $asDto = false): array|\BlackCat\Database\Packages\RateLimitCounters\Dto\RateLimitCounterDto|null {
        return $this->getByUnique([ 'subject_type' => $subjectType, 'subject_id' => $subjectId, 'name' => $name, 'window_start' => $windowStart, 'window_size_sec' => $windowSizeSec ], $asDto);
    }
    ...
    public function getIdBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec(string $subjectType, string $subjectId, string $name, \DateTimeInterface|string $windowStart, int $windowSizeSec) {
        $row = $this->getBySubjectTypeAndSubjectIdAndNameAndWindowStartAndWindowSizeSec($subjectType, $subjectId, $name, $windowStart, $windowSizeSec, false);
